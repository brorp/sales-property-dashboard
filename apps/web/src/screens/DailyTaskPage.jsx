...
export default function DailyTaskPage() {
    ...
    const loadSideData = useCallback(async ({ silent = false } = {}) => {
        ...
        try {
            const [appointmentData, hotData, cancelReasonData] = await Promise.all([
    ...
    const actionableTasks = (items) => (Array.isArray(items) ? items : []).filter((item) => item.salesStatus !== 'skip');
    const newLeadCount = actionableTasks(tasks.newLeads).length;
    const followUpCount = actionableTasks(tasks.followUps).length;
    const deadlineLeadCount = actionableTasks(tasks.deadlineLeads).length;
    const activeAppointments = appointments.filter((item) => item.status === 'mau_survey');
    const visibleValidatedHot = validatedHot.filter((lead) => !hasFilledResultStatus(lead));
            ...
            <div className="daily-task-tabs">
                <button type="button" className={`daily-task-tab${activeTab === 'new_leads' ? ' is-active' : ''}`} onClick={() => { setActiveTab('new_leads'); setNameSearch(''); setHotSubTab('semua'); }}>
                    <span className="daily-task-tab-icon-wrap">
                        ...
                        <span className="daily-task-tab-badge" style={newLeadCount === 0 ? { visibility: 'hidden' } : undefined}>{newLeadCount}</span>
                    </span>
                ...
                <button type="button" className={`daily-task-tab${activeTab === 'hot_validated' ? ' is-active' : ''}`} onClick={() => { setActiveTab('hot_validated'); setNameSearch(''); setHotSubTab('semua'); }}>
                    <span className="daily-task-tab-icon-wrap">
                        ...
                        <span className="daily-task-tab-badge" style={visibleValidatedHot.length === 0 ? { visibility: 'hidden' } : undefined}>{visibleValidatedHot.length}</span>
                    </span>

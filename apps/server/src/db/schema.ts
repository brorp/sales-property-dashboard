...
export const dailyTask = pgTable(
    "daily_task",
    {
        ...
        salesId: text("sales_id")
            ...
            .references(() => user.id, { onDelete: "cascade" }),
        clientId: text("client_id")
            .references(() => client.id, { onDelete: "set null" }),
        ...
        submittedSalesStatus: text("submitted_sales_status"),
        note: text("note"),
        createdAt: timestamp("created_at").notNull().defaultNow(),
        updatedAt: timestamp("updated_at").notNull().defaultNow(),
    },
    (table) => ({
        leadIdx: index("daily_task_lead_id_idx").on(table.leadId),
        salesIdx: index("daily_task_sales_id_idx").on(table.salesId),
        clientIdx: index("daily_task_client_id_idx").on(table.clientId),
        statusDueIdx: index("daily_task_status_due_idx").on(table.status, table.dueAt),
        uniqueTaskPerLead: uniqueIndex("daily_task_lead_type_stage_unique").on(
            table.leadId,
            table.taskType,
            table.followupStage
        ),
...
export const dailyTaskPenalty = pgTable(
    "daily_task_penalty",
    {
        id: text("id").primaryKey(),
        salesId: text("sales_id")
        ...
        penaltySequence: integer("penalty_sequence").notNull().default(1),
        blockedFrom: timestamp("blocked_from").notNull(),
        blockedUntil: timestamp("blocked_until").notNull(),
        durationHours: integer("duration_hours").notNull().default(24),
        spLevel: text("sp_level").notNull().default("none"),
        status: text("status").notNull().default("active"),
        compensatedBy: text("compensated_by").references(() => user.id, { onDelete: "set null" }),
    ...
    (table) => ({
        salesIdx: index("daily_task_penalty_sales_id_idx").on(table.salesId),
        clientIdx: index("daily_task_penalty_client_id_idx").on(table.clientId),
        statusIdx: index("daily_task_penalty_status_idx").on(table.status, table.blockedUntil),
        taskUnique: uniqueIndex("daily_task_penalty_task_id_unique").on(table.taskId),
    })
);

export const dailyTaskPenaltySuspension = pgTable(
    "daily_task_penalty_suspension",
    {
        id: text("id").primaryKey(),
        salesId: text("sales_id")
            .notNull()
        ...
        penaltyId: text("penalty_id")
            .notNull()
            .references(() => dailyTaskPenalty.id, { onDelete: "cascade" }),
        suspendedFrom: timestamp("suspended_from").notNull(),
    ...
    (table) => ({
        salesIdx: index("daily_task_penalty_suspension_sales_id_idx").on(table.salesId),
        clientIdx: index("daily_task_penalty_suspension_client_id_idx").on(table.clientId),
        statusIdx: index("daily_task_penalty_suspension_status_idx").on(
            table.status,
            table.suspendedUntil
        ),
        penaltyUnique: uniqueIndex("daily_task_penalty_suspension_penalty_id_unique").on(
            table.penaltyId
        ),
    })
);
